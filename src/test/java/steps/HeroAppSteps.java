...
public class HeroAppSteps {
    ...
    public void userShouldSeeHeading(String headingText) {
        switch (headingText){
            ...
            case "Context Menu":
                ...
                break;
            case "Entry Ad":
                Assert.assertTrue(heroAppPage.entryAdHeading.isDisplayed());
                Assert.assertEquals(headingText, heroAppPage.entryAdHeading.getText());
                break;
            default:
    ...
    public void userShouldSeeText(String paragraphText) {
        switch (paragraphText){
            ...
            case "Right-click in the box below to see one called 'the-internet'. When you click it, it will trigger a JavaScript alert.":
                ...
                break;
            case "Displays an ad on page load.":
                Assert.assertTrue(heroAppPage.entryAdParagraph1.isDisplayed());
                Assert.assertEquals(paragraphText, heroAppPage.entryAdParagraph1.getText());
                break;
            case "If closed, it will not appear on subsequent page loads.":
                Assert.assertTrue(heroAppPage.entryAdParagraph2.isDisplayed());
                Assert.assertEquals(paragraphText, heroAppPage.entryAdParagraph2.getText());
                break;
            default:
    ...
    public void userShouldSeeAMessageStartsWith(String errorMessage) {
        Assert.assertTrue(heroAppPage.errorMessage.getText().startsWith(errorMessage));
    }

    @When("user clicks on {string}")
    public void userClicksOn(String link) {
        heroAppPage.entryAdClickHereLink.click();
        Waiter.pause(2);
    }

    @Then("user should see a message {string}")
    public void userShouldSeeAMessage(String popupMessage) {
        Assert.assertTrue(heroAppPage.entryAdPopupModalMessage.isDisplayed());
        Assert.assertEquals(popupMessage, heroAppPage.entryAdPopupModalMessage.getText());
