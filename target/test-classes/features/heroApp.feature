...
Feature: HeroApp Functionalities
  ...
  Scenario: Validate entry ad page
    When user clicks on "Entry Ad" link
    Then user should see "Entry Ad" heading
    And user should see "Displays an ad on page load." text
    And user should see "If closed, it will not appear on subsequent page loads." text
    When user clicks on "click here"
    Then user should see a message "THIS IS A MODAL WINDOW"
